...
const Admeditservice = () => {
    ...
    useEffect(() => {
        const fetchServiceData = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_EDIT_SERVICE}/${idservices}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url1 = process.env.REACT_APP_EDIT_SERVICE;
            ...
            if (!response.ok) {
                ...
                if (selectedFile) {
                    ...
                    await fetch(process.env.REACT_APP_GET_IMAGES, {
                        method: 'POST',
                        body: formData
                    });
                }
    ...
    const handleDelete = async () => {
        if (window.confirm("Are you sure you want to delete this service?")) {
            try {
                const url = `${process.env.REACT_APP_EDIT_SERVICE}/${idservices}` + "/" + originalName;
