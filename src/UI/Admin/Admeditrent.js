...
const Admeditrent = () => {
    ...
    useEffect(() => {
        const fetchRentData = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_EDIT_RENT}/${idrents}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url1 = process.env.REACT_APP_EDIT_RENT;
    ...
    const handleDelete = async () => {
        if (window.confirm("Are you sure you want to delete this rent?")) {
            try {
                const url = `${process.env.REACT_APP_EDIT_RENT}/${idrents}` + "/" + originalName;
