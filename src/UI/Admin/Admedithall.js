...
const Admedithall = () => {
    ...
    useEffect(() => {
        const fetchHallData = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_EDIT_HALL}/${idhalls}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url1 = process.env.REACT_APP_EDIT_HALL;
    ...
    const handleDelete = async () => {
        if (window.confirm("Are you sure you want to delete this hall?")) {
            try {
                const url = `${process.env.REACT_APP_EDIT_HALL}/${idhalls}` + "/" + originalName;
